...
def P(ind, tx, ty):
    ...
    B = -0.5             # Probability dependence (slope)
    C = 0.01             # Min probability

    for i in range(ind):
        if abs(x[i, 0] - tx) < L / 2 and abs(x[i, 1] - ty) < L / 2:
            n1 += 1      # count particles within L/2 x L/2 cell surrounding ind
        if abs(x[i, 0] - tx) < L and abs(x[i, 1] - ty) < L:
            n2 += 1      # count particles within L x L cell surrounding ind

    p = A + B * n1 / n2 if n2 != 0 else C  # evaluate probability, avoid division by zero
    if p < C:
        p = C            # enforce minimum probability
