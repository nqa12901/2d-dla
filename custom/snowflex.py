...
N = 500  # Number of Particles in Aggregate
bias = 0.1  # Central tendency of random walkers (increase to speed up simulation)
...
# Particle Array
x = np.zeros((N * 6, 3))  # Create a larger array to accommodate symmetrical points
...
def main():
    ...
    # DLA Start:
    index = n0  # Current index in the particle array
    for i in range(n0, N):
        ...
        # Create symmetrical snowflake pattern by reflecting particles
        x[index, 0] = tx
        x[index, 1] = ty
        for j in range(1, 6):
            angle = j * PI / 3
            x_sym = tx * math.cos(angle) - ty * math.sin(angle)
            y_sym = tx * math.sin(angle) + ty * math.cos(angle)
            x[index + j, 0] = x_sym
            x[index + j, 1] = y_sym

        index += 6  # Move to the next set of positions
